  ...
  <header class="site-header" data-header>
    <nav class="nav-shell" aria-label="主导航">
      ...
      <div class="nav-links" data-nav-links>
        ${Z.map(t=>`<a href="${t.path}" data-route>${t.label}</a>`).join("")}
...
`;const w=document.querySelector("#page-root"),at=document.querySelector("[data-header]"),j=document.querySelector("[data-menu-button]"),J=document.querySelector("[data-nav-links]"),b=document.querySelector("[data-search-modal]"),nt=document.querySelector("[data-search-open]"),ot=document.querySelector("[data-search-close]"),K=document.querySelector("[data-backtop]"),x=document.querySelector("[data-pet]"),M=window.matchMedia("(prefers-reduced-motion: reduce)").matches;D();window.addEventListener("popstate",D);window.addEventListener("scroll",()=>{at?.classList.toggle("is-scrolled",window.scrollY>20),K?.classList.toggle("is-visible",window.scrollY>420)});j?.addEventListener("click",()=>{J?.classList.toggle("is-open"),j.classList.toggle("is-open")});nt?.addEventListener("click",()=>Nt());ot?.addEventListener("click",()=>N());b?.addEventListener("click",t=>{t.target===b&&N()});document.addEventListener("keydown",t=>{t.key==="Escape"&&N()});K?.addEventListener("click",()=>window.scrollTo({top:0,behavior:M?"auto":"smooth"}));x?.addEventListener("click",()=>x.classList.add("is-hidden"));document.addEventListener("click",Lt);document.addEventListener("keydown",Et);document.addEventListener("input",xt);document.addEventListener("change",Tt);document.addEventListener("focusin",Mt);document.addEventListener("focusout",It);document.addEventListener("error",Pt,!0);M||(Rt(),Ut(),Wt());function D(){if(!w)return;const t=window.location.pathname;if(document.body.classList.toggle("is-admin-route",t==="/admin"),it(t),t==="/admin"){w.innerHTML=ct(),Ot(),pt();return}if(t.startsWith("/articles/")){w.innerHTML=lt(),ut(decodeURIComponent(t.replace("/articles/","")));return}if(t!=="/"&&F[t]){w.innerHTML=st(F[t]);return}w.innerHTML=rt(),zt(),Ft(),I()}function it(t){document.querySelectorAll("[data-route]").forEach(e=>{const a=e.pathname===t||t==="/"&&e.pathname==="/";e.classList.toggle("is-active",a)})}function rt(){return`
    ...
    <section class="content-shell" id="posts">
      ...
      <aside class="sidebar" id="about" aria-label="侧边栏">
        ...
        <section class="side-panel">
          <h2>标签</h2>
          <div class="tag-cloud">
            ${et.map(t=>`<a href="#" data-static-action>${t}</a>`).join("")}
          </div>
        </section>
      </aside>
    </section>
  ...
  `}function V(t){const e=t.featured?"post-card featured":"post-card",a=t.slug?`/articles/${encodeURIComponent(t.slug)}`:"#",o=t.slug?"data-route":"";return`
  ...
  `}function bt(t){f={...f,total:t.length,published:t.filter(e=>e.status==="published").length,drafts:t.filter(e=>e.status==="draft").length},X()}function X(){const t=Math.max(f.published,f.drafts,f.media,1),e=(o,n)=>{const i=document.querySelector(o);i&&(i.textContent=String(n))},a=(o,n)=>{const i=document.querySelector(o);i&&i.style.setProperty("--bar-height",`${Math.max(8,n/t*100)}%`)};e("[data-stat-total]",f.total),e("[data-stat-published]",f.published),e("[data-stat-drafts]",f.drafts),e("[data-stat-media]",f.media),a("[data-chart-published]",f.published),a("[data-chart-drafts]",f.drafts),a("[data-chart-media]",f.media)}async function P(){const t=document.querySelector("[data-admin-media-list]");if(t){t.textContent="正在读取...";try{const e=await fetch("/api/admin/uploads",{credentials:"include"});if(!e.ok)throw new Error("Failed to load uploads");const o=(await e.json()).uploads??[];f.media=o.length,X(),t.innerHTML=o.length?o.map(yt).join(""):"<p>还没有上传图片。上传后会出现在这里，可复用为封面或正文图片。</p>"}catch{t.innerHTML="<p>媒体库读取失败，请确认后端已启动并已登录。</p>"}}}function yt(t){return`
    <article class="media-item">
      <img src="${p(t.path)}" alt="${p(t.originalName)}" loading="lazy" />
      <div>
        <strong title="${p(t.originalName)}">${p(t.originalName)}</strong>
        <small>${Ht(t.size)} · ${C(t.createdAt)}</small>
  ...
  `}async function O(t){const e=document.querySelector("[data-article-title]"),a=document.querySelector("[data-article-slug]"),o=document.querySelector("[data-article-category]"),n=document.querySelector("[data-article-summary]"),i=document.querySelector("[data-article-cover]"),r=document.querySelector("[data-md-source]"),s=e?.value.trim()??"",l=jt(a?.value||s),d=o?.value.trim()||"手记",u=r?.value??"";if(!s||!l||!u.trim()){g("标题、Slug 和正文不能为空。","error"),m("标题、Slug 和正文不能为空。","error");return}g(t==="published"?"正在发布...":"正在保存草稿..."),m(t==="published"?"正在发布文章...":"正在保存草稿..."),U(!0);try{const c=await fetch($?`/api/admin/articles/${$}`:"/api/admin/articles",{method:$?"PUT":"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify({title:s,slug:l,category:d,summary:n?.value.trim()??"",coverImage:i?.value.trim()??"",markdown:u,status:t})});if(!c.ok)throw new Error("Save failed");const h=await c.json();$=h.article.id,a.value=h.article.slug,g(t==="published"?"文章已发布。":"草稿已保存。"),m(t==="published"?"文章已发布。":"草稿已保存。"),await S(),t==="published"&&await I(),t==="published"&&B()}catch{g("保存失败。Slug 可能已存在，或登录状态已过期。","error"),m("保存失败。Slug 可能已存在，或登录状态已过期。","error")}finally{U(!1)}}async function vt(t){g("正在上传图片..."),m("正在上传图片..."),z(!0);const e=new FormData;e.append("file",t);try{const a=await fetch("/api/admin/uploads",{method:"POST",credentials:"include",body:e});if(!a.ok)throw new Error("Upload failed");const o=await a.json(),n=o.upload?.path;if(!n)throw new Error("Upload path missing");const i=document.querySelector("[data-article-cover]"),r=document.querySelector("[data-md-source]");i&&!i.value&&(i.value=n),r&&(r.value=`${r.value.trim()}

![${t.name}](${n})`.trim(),k(r.value)),await P();const s=o.reused?"图片已存在，已复用并插入正文。":"图片已上传，并已插入正文。";g(s),m(s)}catch{g("图片上传失败，请确认已登录且文件小于 5MB。","error"),m("图片上传失败，请确认已登录且文件小于 5MB。","error")}finally{z(!1)}}async function wt(t){try{const e=await fetch("/api/admin/articles",{credentials:"include"});if(!e.ok)throw new Error("Failed to load articles");const o=(await e.json()).articles?.find(n=>n.id===t);if(!o)throw new Error("Article not found");St(o),T("editor"),m("已载入文章。")}catch{m("载入文章失败。","error")}}async function $t(t){if(window.confirm("确定删除这篇文章？此操作不可恢复。"))try{if(!(await fetch(`/api/admin/articles/${t}`,{method:"DELETE",credentials:"include"})).ok)throw new Error("Delete failed");$===t&&B(),await S(),await I(),m("文章已删除。")}catch{m("删除失败。","error")}}async function R(t,e){try{if(!(await fetch(`/api/admin/articles/${t}/move`,{method:"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify({direction:e})})).ok)throw new Error("Move failed");await S(),await I(),m(e==="up"?"已上移。":"已下移。")}catch{m("调整顺序失败。","error")}}function St(t){$=t.id,document.querySelector("[data-article-title]").value=t.title,document.querySelector("[data-article-slug]").value=t.slug,document.querySelector("[data-article-category]").value=t.category,document.querySelector("[data-article-summary]").value=t.summary,document.querySelector("[data-article-cover]").value=t.coverImage??"",document.querySelector("[data-md-source]").value=t.markdown;const e=document.querySelector("[data-editor-state]");e&&(e.textContent=`当前：编辑 #${t.id} · ${t.status==="published"?"已发布":"草稿"}`),k(t.markdown)}function T(t){document.querySelectorAll("[data-admin-section]").forEach(e=>{e.hidden=e.dataset.adminSection!==t}),document.querySelectorAll("[data-admin-tab]").forEach(e=>{e.classList.toggle("is-active",e.dataset.adminTab===t)})}function kt(t){const e=document.querySelector("[data-article-cover]");!e||!t||(e.value=t,m("已设为封面图。"))}function At(t,e="image"){const a=document.querySelector("[data-md-source]");!a||!t||(a.value=`${a.value.trim()}

![${e}](${t})`.trim(),k(a.value),m("已插入正文。"))}async function qt(t){if(window.confirm("确定删除这张图片？使用它的文章会显示图片不存在占位图。"))try{if(!(await fetch(`/api/admin/uploads/${t}`,{method:"DELETE",credentials:"include"})).ok)throw new Error("Delete upload failed");await P(),m("图片已删除。")}catch{m("图片删除失败。","error")}}function U(t){document.querySelectorAll("[data-save-draft], [data-publish-article]").forEach(e=>{e.disabled=t,e.classList.toggle("is-busy",t)})}function z(t){document.querySelectorAll("[data-upload-button]").forEach(e=>{e.disabled=t,e.classList.toggle("is-busy",t)})}function B(){$=null,document.querySelectorAll("[data-article-title], [data-article-slug], [data-article-category], [data-article-summary], [data-article-cover], [data-md-source]").forEach(e=>{e.value=""});const t=document.querySelector("[data-editor-state]");t&&(t.textContent="当前：新建文章"),k("")}function Lt(t){const e=t.target;if(!(e instanceof Element))return;const a=e.closest("[data-admin-tab]");if(a){t.preventDefault();const c=a.dataset.adminTab;(c==="articles"||c==="editor"||c==="media")&&T(c);return}if(e.closest("[data-new-article]")){t.preventDefault(),B(),T("editor"),m("已切换为新建文章。");return}if(e.closest("[data-login-button]")){t.preventDefault(),ht();return}if(e.closest("[data-logout-button]")){t.preventDefault(),ft();return}if(e.closest("[data-save-draft]")){t.preventDefault(),O("draft");return}if(e.closest("[data-publish-article]")){t.preventDefault(),O("published");return}if(e.closest("[data-upload-button]")){t.preventDefault(),document.querySelector("[data-upload-file]")?.click();return}const o=e.closest("[data-md-tool]");if(o){t.preventDefault(),Ct(o.dataset.mdTool??"");return}const n=e.closest("[data-article-action]");if(n){t.preventDefault();const c=Number(n.dataset.articleId),h=n.dataset.articleAction;if(h==="view"){const v=n.closest("[data-article-slug]")?.dataset.articleSlug;v&&W(`/articles/${encodeURIComponent(v)}`);return}h==="edit"&&wt(c),h==="delete"&&$t(c),h==="up"&&R(c,"up"),h==="down"&&R(c,"down");return}const i=e.closest("[data-media-cover]");if(i){t.preventDefault(),kt(i.dataset.mediaCover??"");return}const r=e.closest("[data-media-insert]");if(r){t.preventDefault(),At(r.dataset.mediaInsert??"",r.dataset.mediaName??"image");return}const s=e.closest("[data-media-delete]");if(s){t.preventDefault(),qt(Number(s.dataset.mediaDelete));return}const l=e.closest("a[data-route]");if(l){t.preventDefault(),W(l.pathname);return}const d=e.closest('a[href^="#"]');if(d){t.preventDefault(),H(d);const c=d.getAttribute("href")?.slice(1);c&&document.getElementById(c)?.scrollIntoView({behavior:M?"auto":"smooth"});return}const u=e.closest("[data-static-action]");u&&H(u)}function Et(t){if(t.key!=="Enter"&&t.key!==" ")return;const e=t.target;e instanceof HTMLElement&&e.matches("[data-static-action]")&&(t.preventDefault(),H(e))}function xt(t){const e=t.target;if(e instanceof HTMLInputElement&&e.matches("[data-article-search]")){S();return}!(e instanceof HTMLTextAreaElement)||!e.matches("[data-md-source]")||k(e.value)}function Tt(t){const e=t.target;if(!(e instanceof HTMLInputElement)||!e.matches("[data-upload-file]"))return;const a=e.files?.[0];a&&(vt(a),e.value="")}function Mt(t){const e=t.target;e instanceof HTMLElement&&(!e.matches("[data-md-source]")&&!e.closest("[data-md-toolbar]")||document.querySelector(".editor-panel")?.classList.add("has-md-focus"))}function It(){window.setTimeout(()=>{const t=document.activeElement,e=document.querySelector(".editor-panel");!e||t instanceof HTMLElement&&(t.matches("[data-md-source]")||t.closest("[data-md-toolbar]"))||e.classList.remove("has-md-focus")},80)}function Pt(t){const e=t.target;e instanceof HTMLImageElement&&e.dataset.missingImage!=="true"&&(!e.currentSrc.includes("/api/uploads/")&&!e.src.includes("/api/uploads/")||(e.dataset.missingImage="true",e.alt="图片不存在",e.src=Dt()))}function k(t){const e=document.querySelector("[data-md-preview]");e&&(e.innerHTML=Q(t)||"输入 Markdown 后这里会显示预览。")}function Ct(t){const e=document.querySelector("[data-md-source]");if(!e)return;const a=e.selectionStart,o=e.selectionEnd,n=e.value.slice(a,o),i=n||"文本";let r="",s=a,l=a;if(t==="h1"||t==="h2"||t==="h3"){const d=Number(t.slice(1));r=`${"#".repeat(d)} ${i}`,s=a+d+1,l=s+i.length}else if(t==="bold")r=`**${i}**`,s=a+2,l=s+i.length;else if(t==="italic")r=`*${i}*`,s=a+1,l=s+i.length;else if(t==="quote")r=n?n.split(`
`).map(d=>`> ${d}`).join(`
