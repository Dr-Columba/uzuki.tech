...
`;const v=document.querySelector("#page-root"),lt=document.querySelector("[data-header]"),U=document.querySelector("[data-menu-button]"),at=document.querySelector("[data-nav-links]"),$=document.querySelector("[data-search-modal]"),dt=document.querySelector("[data-search-open]"),ut=document.querySelector("[data-search-close]"),nt=document.querySelector("[data-backtop]"),P=document.querySelector("[data-pet]"),E=window.matchMedia("(prefers-reduced-motion: reduce)").matches;z();window.addEventListener("popstate",z);window.addEventListener("scroll",()=>{lt?.classList.toggle("is-scrolled",window.scrollY>20),nt?.classList.toggle("is-visible",window.scrollY>420)});U?.addEventListener("click",()=>{at?.classList.toggle("is-open"),U.classList.toggle("is-open")});dt?.addEventListener("click",()=>ae());ut?.addEventListener("click",()=>J());$?.addEventListener("click",t=>{t.target===$&&J()});document.addEventListener("keydown",t=>{t.key==="Escape"&&J()});nt?.addEventListener("click",()=>window.scrollTo({top:0,behavior:E?"auto":"smooth"}));P?.addEventListener("click",()=>P.classList.add("is-hidden"));document.addEventListener("click",zt);document.addEventListener("keydown",Wt);document.addEventListener("input",Gt);document.addEventListener("change",Vt);document.addEventListener("focusin",Jt);document.addEventListener("focusout",Kt);document.addEventListener("error",Yt,!0);E||(ie(),re(),ce());function z(){if(!v)return;const t=window.location.pathname;if(document.body.classList.toggle("is-admin-route",t==="/admin"),mt(t),t==="/admin"){v.innerHTML=ht(),oe(),At();return}if(t.startsWith("/articles/")){v.innerHTML=bt(),Lt(decodeURIComponent(t.replace("/articles/","")));return}if(t.startsWith("/categories/")){const e=decodeURIComponent(t.replace("/categories/",""));v.innerHTML=N(e,"分类"),L({category:e});return}if(t.startsWith("/tags/")){const e=decodeURIComponent(t.replace("/tags/",""));v.innerHTML=N(e,"标签"),L({tag:e});return}if(F[t]){v.innerHTML=N(F[t],"分类"),L({category:F[t]});return}if(t!=="/"&&K[t]){v.innerHTML=ft(K[t]);return}v.innerHTML=pt(),se(),ne(),L(),wt()}function mt(t){document.querySelectorAll("[data-route]").forEach(e=>{const a=e.pathname===t||t==="/"&&e.pathname==="/";e.classList.toggle("is-active",a)})}function pt(){return`
  ...
  `}async function _(t){const e=document.querySelector("[data-article-title]"),a=document.querySelector("[data-article-slug]"),o=document.querySelector("[data-article-category]"),n=document.querySelector("[data-article-tags]"),r=document.querySelector("[data-article-summary]"),i=document.querySelector("[data-article-cover]"),s=document.querySelector("[data-md-source]"),l=e?.value.trim()??"",d=a?.value.trim()??"",m=Xt(d),c=o?.value.trim()||"手记",h=s?.value??"",f=[];if(!l&&e&&f.push({label:"标题",message:"请填写标题。",field:e}),!d&&a&&f.push({label:"Slug",message:"请填写 Slug。",field:a}),d&&!m&&a&&f.push({label:"Slug",message:"Slug 需包含英文字母、数字或连字符。",field:a}),!h.trim()&&s&&f.push({label:"正文",message:"请填写正文。",field:s}),f.length>0){const g=f.length===1?f[0].message:`请填写：${Array.from(new Set(f.map(b=>b.label))).join("、")}。`;y(g,"error"),p(g,"error"),Ot(f.map(b=>b.field));return}y(t==="published"?"正在发布...":"正在保存草稿..."),p(t==="published"?"正在发布文章...":"正在保存草稿..."),Q(!0);try{const g=await fetch(S?`/api/admin/articles/${S}`:"/api/admin/articles",{method:S?"PUT":"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify({title:l,slug:m,category:c,tags:Qt(n?.value??""),summary:r?.value.trim()??"",coverImage:i?.value.trim()??"",markdown:h,status:t})});if(!g.ok)throw new Error("Save failed");const b=await g.json();S=b.article.id,a.value=b.article.slug,y(t==="published"?"文章已发布。":"草稿已保存。"),p(t==="published"?"文章已发布。":"草稿已保存。"),await A(),t==="published"&&await L(),t==="published"&&V()}catch{y("保存失败。Slug 可能已存在，或登录状态已过期。","error"),p("保存失败。Slug 可能已存在，或登录状态已过期。","error")}finally{Q(!1)}}async function jt(t){y("正在上传图片..."),p("正在上传图片..."),Z(!0);const e=new FormData;e.append("file",t);try{const a=await fetch("/api/admin/uploads",{method:"POST",credentials:"include",body:e});if(!a.ok)throw new Error("Upload failed");const o=await a.json(),n=o.upload?.path;if(!n)throw new Error("Upload path missing");const r=document.querySelector("[data-article-cover]"),i=document.querySelector("[data-md-source]");r&&!r.value&&(r.value=n),i&&(i.value=`${i.value.trim()}

![${t.name}](${n})`.trim(),q(i.value)),await D();const s=o.reused?"图片已存在，已复用并插入正文。":"图片已上传，并已插入正文。";y(s),p(s)}catch{y("图片上传失败，请确认已登录且文件小于 5MB。","error"),p("图片上传失败，请确认已登录且文件小于 5MB。","error")}finally{Z(!1)}}async function Dt(t){try{const e=await fetch("/api/admin/articles",{credentials:"include"});if(!e.ok)throw new Error("Failed to load articles");const o=(await e.json()).articles?.find(n=>n.id===t);if(!o)throw new Error("Article not found");Ft(o),j("editor"),p("已载入文章。")}catch{p("载入文章失败。","error")}}async function Bt(t){if(window.confirm("确定删除这篇文章？此操作不可恢复。"))try{if(!(await fetch(`/api/admin/articles/${t}`,{method:"DELETE",credentials:"include"})).ok)throw new Error("Delete failed");S===t&&V(),await A(),await L(),p("文章已删除。")}catch{p("删除失败。","error")}}async function X(t,e){try{if(!(await fetch(`/api/admin/articles/${t}/move`,{method:"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify({direction:e})})).ok)throw new Error("Move failed");await A(),await L(),p(e==="up"?"已上移。":"已下移。")}catch{p("调整顺序失败。","error")}}function Ft(t){S=t.id,document.querySelector("[data-article-title]").value=t.title,document.querySelector("[data-article-slug]").value=t.slug,document.querySelector("[data-article-category]").value=t.category,document.querySelector("[data-article-tags]").value=(t.tags??[]).join(", "),document.querySelector("[data-article-summary]").value=t.summary,document.querySelector("[data-article-cover]").value=t.coverImage??"",document.querySelector("[data-md-source]").value=t.markdown;const e=document.querySelector("[data-editor-state]");e&&(e.textContent=`当前：编辑 #${t.id} · ${t.status==="published"?"已发布":"草稿"}`),q(t.markdown)}function j(t){document.querySelectorAll("[data-admin-section]").forEach(e=>{e.hidden=e.dataset.adminSection!==t}),document.querySelectorAll("[data-admin-tab]").forEach(e=>{e.classList.toggle("is-active",e.dataset.adminTab===t)}),t==="system"?(W(),It()):G()}function Nt(t){const e=document.querySelector("[data-article-cover]");!e||!t||(e.value=t,p("已设为封面图。"))}function Rt(t,e="image"){const a=document.querySelector("[data-md-source]");!a||!t||(a.value=`${a.value.trim()}

![${e}](${t})`.trim(),q(a.value),p("已插入正文。"))}async function Ut(t){if(window.confirm("确定删除这张图片？使用它的文章会显示图片不存在占位图。"))try{if(!(await fetch(`/api/admin/uploads/${t}`,{method:"DELETE",credentials:"include"})).ok)throw new Error("Delete upload failed");await D(),p("图片已删除。")}catch{p("图片删除失败。","error")}}function Q(t){document.querySelectorAll("[data-save-draft], [data-publish-article]").forEach(e=>{e.disabled=t,e.classList.toggle("is-busy",t)})}function Z(t){document.querySelectorAll("[data-upload-button]").forEach(e=>{e.disabled=t,e.classList.toggle("is-busy",t)})}function V(){S=null,document.querySelectorAll("[data-article-title], [data-article-slug], [data-article-category], [data-article-tags], [data-article-summary], [data-article-cover], [data-md-source]").forEach(e=>{e.value="",rt(e)});const t=document.querySelector("[data-editor-state]");t&&(t.textContent="当前：新建文章"),q("")}function Ot(t){t.forEach(a=>{a.setAttribute("aria-invalid","true"),a.classList.remove("field-validation-pulse"),a.offsetWidth,a.classList.add("field-validation-pulse")});const e=t[0];e.scrollIntoView({behavior:E?"auto":"smooth",block:"center"}),e.focus({preventScroll:!0})}function rt(t){t.removeAttribute("aria-invalid"),t.classList.remove("field-validation-pulse")}function zt(t){const e=t.target;if(!(e instanceof Element))return;const a=e.closest("[data-admin-tab]");if(a){t.preventDefault();const c=a.dataset.adminTab;(c==="articles"||c==="editor"||c==="media"||c==="system")&&j(c);return}if(e.closest("[data-system-refresh]")){t.preventDefault(),W();return}if(e.closest("[data-new-article]")){t.preventDefault(),V(),j("editor"),p("已切换为新建文章。");return}if(e.closest("[data-login-button]")){t.preventDefault(),qt();return}if(e.closest("[data-logout-button]")){t.preventDefault(),Mt();return}if(e.closest("[data-save-draft]")){t.preventDefault(),_("draft");return}if(e.closest("[data-publish-article]")){t.preventDefault(),_("published");return}if(e.closest("[data-upload-button]")){t.preventDefault(),document.querySelector("[data-upload-file]")?.click();return}const o=e.closest("[data-md-tool]");if(o){t.preventDefault(),_t(o.dataset.mdTool??"");return}const n=e.closest("[data-article-action]");if(n){t.preventDefault();const c=Number(n.dataset.articleId),h=n.dataset.articleAction;if(h==="view"){const g=n.closest("[data-article-slug]")?.dataset.articleSlug;g&&tt(`/articles/${encodeURIComponent(g)}`);return}h==="edit"&&Dt(c),h==="delete"&&Bt(c),h==="up"&&X(c,"up"),h==="down"&&X(c,"down");return}const r=e.closest("[data-media-cover]");if(r){t.preventDefault(),Nt(r.dataset.mediaCover??"");return}const i=e.closest("[data-media-insert]");if(i){t.preventDefault(),Rt(i.dataset.mediaInsert??"",i.dataset.mediaName??"image");return}const s=e.closest("[data-media-delete]");if(s){t.preventDefault(),Ut(Number(s.dataset.mediaDelete));return}const l=e.closest("a[data-route]");if(l){t.preventDefault(),tt(l.pathname);return}const d=e.closest('a[href^="#"]');if(d){t.preventDefault(),O(d);const c=d.getAttribute("href")?.slice(1);c&&document.getElementById(c)?.scrollIntoView({behavior:E?"auto":"smooth"});return}const m=e.closest("[data-static-action]");m&&O(m)}function Wt(t){if(t.key!=="Enter"&&t.key!==" ")return;const e=t.target;e instanceof HTMLElement&&e.matches("[data-static-action]")&&(t.preventDefault(),O(e))}function Gt(t){const e=t.target;if((e instanceof HTMLInputElement||e instanceof HTMLTextAreaElement)&&e.matches("[data-article-title], [data-article-slug], [data-md-source]")&&rt(e),e instanceof HTMLInputElement&&e.matches("[data-article-search]")){A();return}!(e instanceof HTMLTextAreaElement)||!e.matches("[data-md-source]")||q(e.value)}function Vt(t){const e=t.target;if(!(e instanceof HTMLInputElement)||!e.matches("[data-upload-file]"))return;const a=e.files?.[0];a&&(jt(a),e.value="")}function Jt(t){const e=t.target;e instanceof HTMLElement&&(!e.matches("[data-md-source]")&&!e.closest("[data-md-toolbar]")||document.querySelector(".editor-panel")?.classList.add("has-md-focus"))}function Kt(){window.setTimeout(()=>{const t=document.activeElement,e=document.querySelector(".editor-panel");!e||t instanceof HTMLElement&&(t.matches("[data-md-source]")||t.closest("[data-md-toolbar]"))||e.classList.remove("has-md-focus")},80)}function Yt(t){const e=t.target;e instanceof HTMLImageElement&&e.dataset.missingImage!=="true"&&(!e.currentSrc.includes("/api/uploads/")&&!e.src.includes("/api/uploads/")||(e.dataset.missingImage="true",e.alt="图片不存在",e.src=te()))}function q(t){const e=document.querySelector("[data-md-preview]");e&&(e.innerHTML=st(t)||"输入 Markdown 后这里会显示预览。")}function _t(t){const e=document.querySelector("[data-md-source]");if(!e)return;const a=e.selectionStart,o=e.selectionEnd,n=e.value.slice(a,o),r=n||"文本";let i="",s=a,l=a;if(t==="h1"||t==="h2"||t==="h3"){const d=Number(t.slice(1));i=`${"#".repeat(d)} ${r}`,s=a+d+1,l=s+r.length}else if(t==="bold")i=`**${r}**`,s=a+2,l=s+r.length;else if(t==="italic")i=`*${r}*`,s=a+1,l=s+r.length;else if(t==="quote")i=n?n.split(`
`).map(d=>`> ${d}`).join(`
